...
{
    //XXLOGD(WebCore::HTML5, "FullscreenElementStack::requestFullScreen - element:%p...#1", element);
    // Ignore this request if the document is not in a live frame.
    if (!document()->isActive())
        return;
    //XXLOGD(WebCore::HTML5, "FullscreenElementStack::requestFullScreen - requestType:%d...#1", requestType);
    //TODO: It should be more appropriate to employ !!(dynamic_cast<HTMLMediaElement*>(element))
    if (Fullscreen::PrefixedVideoRequest == requestType) {
        //XXLOGD(WebCore::HTML5, "document()->frameHost()->chrome().client().enterFullScreenForElement...#2");
