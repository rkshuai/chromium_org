...
public class ContentVideoView extends FrameLayout
    ...
    private static final String LOGTAG = "html5";
   ...
   public static boolean is_default_fullscreen_video = true;
    ...
    private static class ProgressView extends LinearLayout {
        ...
        public ProgressView(Context context, String videoLoadingText) {
            ...
            if(is4k) {
                ...
                1920,//to match the 1080 video width at 4k
                Gravity.LEFT
                ));
            } else {
            setLayoutParams(new LinearLayout.LayoutParams(
    ...
    protected ContentVideoView(Context context, long nativeContentVideoView,
        ...
        //get from native
        is_default_fullscreen_video = nativeIsDefaultFullScreen(mNativeContentVideoView);
    ...
    protected void showContentVideoView() {
        ...
        Log.d(LOGTAG, "VideoSurfaceView - showContentVideoView...");
        ...
        this.addView(mProgressView, new FrameLayout.LayoutParams(
                ...
                ViewGroup.LayoutParams.WRAP_CONTENT,
                Gravity.CENTER));
        //fix for loading video without prompt
        mProgressView.setVisibility(View.VISIBLE);

        //-- set surface type as hisi-digging mode for HiPlayer video output!!
        if(is_default_fullscreen_video){
            Log.d(LOGTAG, "VideoSurfaceView - SurfaceHolder.setType(SURFACE_TYPE_HISI_TRANSPARENT)...");
            //mVideoSurfaceView.getHolder().setType(SurfaceHolder.SURFACE_TYPE_HISI_TRANSPARENT);
            mVideoSurfaceView.getHolder().setType(100);
    ...
    protected void openVideo() {
        ...
        Log.d(LOGTAG, "ContentVideoView - openVideo...");
    ...
    protected void onCompletion() {
        Log.d(LOGTAG, "ContentVideoView - onCompletion...");
    ...
    protected void start() {
        Log.d(LOGTAG, "ContentVideoView - start...");
    ...
    protected void pause() {
        Log.d(LOGTAG, "ContentVideoView - pause...");
        if (isInPlaybackState()) {
            if (isPlaying()) {
                if (mNativeContentVideoView != 0) {
    ...
    private static ContentVideoView createContentVideoView(
        ...
        ContentVideoView videoView = new ContentVideoViewLegacy(context, nativeContentVideoView, client);
    ...
    public void exitFullscreen(boolean relaseMediaPlayer) {
        Log.d(LOGTAG, "ContentVideoView - exitFullscreen--" + relaseMediaPlayer);

        if (mNativeContentVideoView != 0) {
            Log.d(LOGTAG, "ContentVideoView - exitFullscreen -- nativeExitFullscreen & pause -- " + relaseMediaPlayer);
            if(is_default_fullscreen_video){
                nativePause(mNativeContentVideoView);
            }
        ...
        /*destroyContentVideoView should be called here to ensure relaseMediaPlayer flag = true work fine,
           as the custrom client always set it's value to false*/
    ...
    public boolean onKeyUp(int keyCode, KeyEvent event) {
        if (keyCode == KeyEvent.KEYCODE_BACK) {
            //exitFullscreen(false);
            exitFullscreen(is_default_fullscreen_video);
            return true;
        }
    ...
    private native boolean nativeIsDefaultFullScreen(long nativeContentVideoView);
